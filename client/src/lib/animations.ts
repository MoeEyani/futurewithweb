...
/**
 * Creates a smooth transition effect when switching languages
 * @param container The container element that holds the content being translated
 * @param direction 'ltr' for English to Arabic, 'rtl' for Arabic to English
 * @param duration Animation duration in seconds
 * @returns A GSAP timeline that can be controlled
 */
export const languageTransition = (container: HTMLElement, direction: 'ltr' | 'rtl', duration: number = 0.8) => {
  // Create a temporary overlay to add visual effects
  ...
  overlay.style.background = 'linear-gradient(135deg, rgba(78, 137, 174, 0.1), rgba(78, 137, 174, 0.2))';
  ...
  const textElements = container.querySelectorAll('h1, h2, h3, h4, h5, h6, p, li, span, a, button, label, input, textarea');
  
  // Add glowing dots along the edges that will animate during transition
  const dotsCount = 10;
  const dots: HTMLDivElement[] = [];
  
  for (let i = 0; i < dotsCount; i++) {
    const dot = document.createElement('div');
    dot.className = 'language-transition-dot';
    dot.style.position = 'absolute';
    dot.style.width = '6px';
    dot.style.height = '6px';
    dot.style.borderRadius = '50%';
    dot.style.background = 'rgba(78, 137, 174, 0.8)';
    dot.style.boxShadow = '0 0 8px rgba(78, 137, 174, 0.8)';
    dot.style.zIndex = '101';
    dot.style.opacity = '0';
    
    // Position dots around the edges of the container
    const progress = i / dotsCount;
    const angle = progress * Math.PI * 2;
    const radius = Math.min(container.offsetWidth, container.offsetHeight) * 0.5;
  ...
  const tl = gsap.timeline({
    onComplete: () => {
      // Remove the overlay and dots after animation completes
      setTimeout(() => {
  ...
  gsap.set(container, {
    transformOrigin: direction === 'rtl' ? 'left center' : 'right center'
  });
  
  // Phase 1: Begin transition
  // Fade overlay in and add subtle blur
  tl.to(overlay, {
    opacity: 1,
  ...
  // Fade out text and slide based on language direction
  tl.to(textElements, {
    opacity: 0,
  ...
  // Add a subtle scale and rotation effect to the container
  tl.to(container, {
    scale: 0.98,
    rotation: direction === 'rtl' ? -0.5 : 0.5,
    duration: duration / 4,
    ease: 'power2.in'
  }, '<');
  
  // Phase 2: Midpoint for language change
  // Add a callback for when the content should be swapped (language change happens here)
  tl.call(() => {
    // The actual content swap should be handled by React's rendering system
    // This is just a timing marker for when it should happen
  });
  
  // Phase 3: Complete transition
  // Scale back and reset rotation
  tl.to(container, {
    scale: 1,
    rotation: 0,
    duration: duration / 4,
  ...
  // Fade in text from the opposite direction
  tl.to(textElements, {
    opacity: 1,
    x: 0,
    filter: 'blur(0px)',
  ...
  // Animate dots again but in reverse order
  tl.to(dots, {
    opacity: 0,
    stagger: {
      each: 0.02,
